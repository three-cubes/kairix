...
from kairix.quality.eval.metrics import hit_at_k_graded, ndcg_graded, reciprocal_rank_graded

# ---------------------------------------------------------------------------
# ndcg_graded (was ndcg_graded)
# ---------------------------------------------------------------------------
...
class TestComputeNDCG:
    ...
    def test_empty_gold(self):
        assert ndcg_graded(["/a.md"], [], k=10) == pytest.approx(0.0)
    ...
    def test_known_ndcg_value(self):
        ...
        assert ndcg_graded(retrieved, gold, k=10) == pytest.approx(expected, rel=1e-6)
    ...
    def test_k_truncation(self):
        ...
        assert ndcg_graded(retrieved, gold, k=2) == pytest.approx(0.0)

    @pytest.mark.unit
    def test_empty_retrieved(self):
        gold = [{"title": "a", "relevance": 1}]
        assert ndcg_graded([], gold, k=10) == pytest.approx(0.0)


# ---------------------------------------------------------------------------
# hit_at_k_graded (was hit_at_k_graded)
# ---------------------------------------------------------------------------
...
class TestComputeHitAtK:
    ...
    def test_hit_in_top1(self):
        ...
        assert hit_at_k_graded(["/target.md"], gold, k=5) is True

    @pytest.mark.unit
    def test_hit_at_boundary(self):
        gold = [{"title": "target", "relevance": 1}]
        retrieved = ["/a.md", "/b.md", "/c.md", "/d.md", "/target.md"]
        assert hit_at_k_graded(retrieved, gold, k=5) is True
    ...
    def test_no_gold(self):
        assert hit_at_k_graded(["/a.md"], [], k=5) is False

    @pytest.mark.unit
    def test_k1(self):
        gold = [{"title": "target", "relevance": 2}]
        assert hit_at_k_graded(["/target.md"], gold, k=1) is True
        assert hit_at_k_graded(["/other.md", "/target.md"], gold, k=1) is False


# ---------------------------------------------------------------------------
# reciprocal_rank_graded (was reciprocal_rank_graded)
# ---------------------------------------------------------------------------
...
class TestComputeMRR:
    ...
    def test_first_position(self):
        gold = [{"title": "target", "relevance": 1}]
        assert reciprocal_rank_graded(["/target.md"], gold, k=10) == pytest.approx(1.0)

    @pytest.mark.unit
    def test_second_position(self):
        gold = [{"title": "target", "relevance": 1}]
        assert reciprocal_rank_graded(["/other.md", "/target.md"], gold, k=10) == pytest.approx(0.5)

    @pytest.mark.unit
    def test_third_position(self):
        gold = [{"title": "target", "relevance": 1}]
        retrieved = ["/a.md", "/b.md", "/target.md"]
        assert reciprocal_rank_graded(retrieved, gold, k=10) == pytest.approx(1.0 / 3)
    ...
    def test_beyond_k(self):
        gold = [{"title": "target", "relevance": 1}]
        retrieved = ["/a.md", "/b.md", "/target.md"]
        assert reciprocal_rank_graded(retrieved, gold, k=2) == pytest.approx(0.0)

    @pytest.mark.unit
    def test_empty_gold(self):
        assert reciprocal_rank_graded(["/a.md"], [], k=10) == pytest.approx(0.0)
