"""Unit tests for :class:`kairix.providers.azure_legacy.AzureLegacyProvider`.
...
- Endpoint passthrough — Azure-legacy does NOT append ``/openai/v1`` to
  the configured endpoint (that's Foundry-specific).
- Foundry-endpoint rejection — constructing the legacy plugin against a
  Foundry-shaped URL raises :class:`ValueError` with an actionable hint
  naming ``provider: azure_foundry`` as the fix.
- ``api-version`` resolution — default pinned constant, constructor
  override, and the future-extension hook for a ``credentials.api_version``
  attribute.
- Error mapping — every status code (429 / 401 / 403 / 500 / 503) and
  connection failure maps to the canonical typed error; Retry-After
...
from kairix.providers.azure_legacy import (
...
class _RecordingTransportClient:
    ...
    Mirrors the openai-SDK ``AzureOpenAI`` surface that
    :class:`AzureLegacyProvider` actually consumes: an
...
def _legacy_credentials(
    *,
    api_key: str = "legacy-test-key",  # pragma: allowlist secret
    endpoint: str = "https://example-resource.openai.azure.com",
    model: str = "text-embedding-3-large",
    dims: int = 1536,
) -> Credentials:
    """Construct a Credentials test instance pinned to the legacy Azure shape."""
...
class TestProtocolConformance:
    """AzureLegacyProvider satisfies the runtime-checkable Provider Protocol."""

    @pytest.mark.unit
    def test_isinstance_provider_at_runtime(self) -> None:
        # Sabotage-proof: removing any of name / embed_batch / chat /
        # dimension / healthcheck from AzureLegacyProvider breaks the
    ...
    def test_name_matches_pyproject_entry_point_key(self) -> None:
        # Sabotage-proof: if PROVIDER_NAME drifted from "azure_legacy",
        # the pyproject.toml entry-point and BDD feature names would
        # stop matching what get_provider("azure_legacy") returns.
        assert PROVIDER_NAME == "azure_legacy"
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(),
            transport_client=_RecordingTransportClient(),
        )
...
# ---------------------------------------------------------------------------
# Foundry-endpoint rejection
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestFoundryEndpointRejection:
    """Construction-time fail-fast for Foundry-shaped endpoints."""

    @pytest.mark.unit
    def test_foundry_host_fragment_rejected_with_actionable_hint(self) -> None:
        # Sabotage-proof: drop the host-fragment branch in
        # _is_foundry_shaped → the Foundry URL would be accepted silently
        # and the AzureOpenAI client would 404 against it. Caught here.
        creds = _legacy_credentials(endpoint="https://example-resource.services.ai.azure.com")
        with pytest.raises(ValueError) as exc_info:
            AzureLegacyProvider(credentials=creds)
        message = str(exc_info.value)
        assert "provider: azure_foundry" in message, (
            f"expected actionable hint naming 'provider: azure_foundry'; got: {message}"
        )

    @pytest.mark.unit
    def test_openai_v1_suffix_rejected_with_actionable_hint(self) -> None:
        # Sabotage-proof: drop the suffix branch in _is_foundry_shaped →
        # the Foundry alias URL would pass, then AzureOpenAI would
        # double-suffix and 404. Caught here.
        creds = _legacy_credentials(endpoint="https://example-resource.services.ai.azure.com/openai/v1")
        with pytest.raises(ValueError) as exc_info:
            AzureLegacyProvider(credentials=creds)
        message = str(exc_info.value)
        assert "provider: azure_foundry" in message
    ...
    @pytest.mark.unit
    def test_legacy_endpoint_accepted_unchanged(self) -> None:
        # Sabotage-proof: if the rejector flipped to also reject legacy
        # endpoints, this construction would raise — caught here.
        creds = _legacy_credentials(endpoint="https://example-resource.openai.azure.com")
        provider = AzureLegacyProvider(credentials=creds, transport_client=_RecordingTransportClient())
        assert provider.name == "azure_legacy"
...
class TestApiVersionResolution:
    """The api-version override chain honours kwarg > attr > default."""

    @pytest.mark.unit
    def test_default_api_version_pinned_to_adr_constant(self) -> None:
        # Sabotage-proof: if the pinned default mutated from the ADR-
        # documented value, this assertion fails. The exact value flows
        # into the wire query string asserted by the BDD scenario
        # "defaults to the value pinned by the ADR".
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(),
            transport_client=_RecordingTransportClient(),
        )
    ...
    @pytest.mark.unit
    def test_constructor_override_wins_over_default(self) -> None:
        # Sabotage-proof: if _resolve_api_version stopped honouring the
        # kwarg, the pinned default would leak through and this fails.
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(),
    ...
    @pytest.mark.unit
    def test_credentials_attribute_used_when_kwarg_absent(self) -> None:
        # Sabotage-proof: if _resolve_api_version stopped checking the
        # credentials attribute, an extension Credentials subclass
        # carrying api_version wouldn't propagate.

        @dataclass(frozen=True)
        class _CredentialsWithVersion(Credentials):
            api_version: str = ""

        creds = _CredentialsWithVersion(
...
class TestEmbedBatchWireShape:
    ...
    def test_records_model_input_and_dimensions(self) -> None:
        ...
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(model="text-embedding-3-small", dims=1536),
    ...
    def test_empty_input_returns_empty_without_calling_transport(self) -> None:
        ...
        provider = AzureLegacyProvider(credentials=_legacy_credentials(), transport_client=client)

        assert provider.embed_batch([]) == []
        assert client.embeddings.calls == []

    @pytest.mark.unit
    def test_endpoint_is_passed_through_no_foundry_suffix_appended(self) -> None:
        # Sabotage-proof: if the legacy plugin ever started appending
        # ``/openai/v1`` (the Foundry-specific suffix) to the configured
        # endpoint, healthcheck() would surface that mangled URL and
        # legacy Azure operators would 404. Pin the passthrough.
        plain = "https://example-resource.openai.azure.com"
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(endpoint=plain),
            transport_client=_RecordingTransportClient(),
        )
        health = provider.healthcheck()
        assert health.endpoint == plain
        assert "/openai/v1" not in health.endpoint
...
class TestChatWireShape:
    ...
    def test_records_model_messages_and_max_tokens(self) -> None:
        ...
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(model="gpt-4o-mini"),
            transport_client=client,
        )
    ...
    def test_chat_with_none_content_returns_empty_string(self) -> None:
        ...
        provider = AzureLegacyProvider(credentials=_legacy_credentials(), transport_client=_NoneClient())
...
class TestEmbedErrorMapping:
    ...
    def test_401_maps_to_auth_error_naming_provider(self) -> None:
        ...
        # provider_azure_legacy.feature §"401 maps to AuthError" fails.
...
class TestDimensionAndHealth:
    ...
    def test_dimension_defaults_when_credentials_have_no_dims(self) -> None:
        ...
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(dims=0),
            transport_client=_RecordingTransportClient(),
        )
    ...
    def test_dimension_uses_credential_dims_before_embed(self) -> None:
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(dims=2048),
            transport_client=_RecordingTransportClient(),
        )
    ...
    def test_dimension_uses_observed_dim_after_first_embed(self) -> None:
        ...
        provider = AzureLegacyProvider(
            credentials=_legacy_credentials(dims=1536),
            transport_client=client,
        )
    ...
    def test_healthcheck_ok_when_embed_succeeds(self) -> None:
        ...
        assert health.endpoint == "https://example-resource.openai.azure.com"
