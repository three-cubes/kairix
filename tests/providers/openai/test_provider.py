"""Unit tests for :class:`kairix.providers.openai.OpenAIProvider`.
...
- Endpoint passthrough — OpenAI-direct does NOT append ``/openai/v1`` to
  the configured base URL (distinct from azure_foundry).
...
from kairix.providers.openai import (
...
class _RecordingTransportClient:
    ...
    Mirrors the openai-SDK surface that :class:`OpenAIProvider` actually
    consumes: an ``embeddings.create`` and a ``chat.completions.create``
    method. No HTTP is performed; the recorded kwargs are the wire-shape
...
def _openai_credentials(
    *,
    api_key: str = "openai-test-key",  # pragma: allowlist secret
    endpoint: str = "https://api.openai.com/v1",
    model: str = "text-embedding-3-large",
    dims: int = 1536,
) -> Credentials:
    """Construct a Credentials test instance pinned to OpenAI-direct shape."""
...
class TestProtocolConformance:
    """OpenAIProvider satisfies the runtime-checkable Provider Protocol."""

    @pytest.mark.unit
    def test_isinstance_provider_at_runtime(self) -> None:
        # Sabotage-proof: removing any of name / embed_batch / chat /
        # dimension / healthcheck from OpenAIProvider breaks the
    ...
    def test_name_matches_pyproject_entry_point_key(self) -> None:
        # Sabotage-proof: if PROVIDER_NAME drifted from "openai", the
        # pyproject.toml entry-point and BDD feature names would stop
        # matching what get_provider("openai") returns.
...
class TestEmbedBatchWireShape:
    ...
    def test_records_model_input_and_dimensions(self) -> None:
        ...
        provider = OpenAIProvider(
            credentials=_openai_credentials(model="text-embedding-3-small", dims=1536),
    ...
    def test_empty_input_returns_empty_without_calling_transport(self) -> None:
        ...
        provider = OpenAIProvider(credentials=_openai_credentials(), transport_client=client)
    ...
    def test_endpoint_is_passed_through_unchanged_no_suffix_appended(self) -> None:
        # Sabotage-proof: if the openai plugin started appending
        # ``/openai/v1`` (the Foundry-specific suffix) to the configured
        # base URL, healthcheck() would surface that mangled URL and
        # operators with custom base URLs (Together, Groq, vLLM) would
        # hit 404s. This test pins the endpoint passthrough.
        plain = "https://api.openai.com/v1"
        provider = OpenAIProvider(
            credentials=_openai_credentials(endpoint=plain),
            transport_client=_RecordingTransportClient(),
        )
...
class TestChatWireShape:
    ...
    def test_records_model_messages_and_max_tokens(self) -> None:
        ...
        provider = OpenAIProvider(
            credentials=_openai_credentials(model="gpt-4o-mini"),
            transport_client=client,
        )
    ...
    def test_chat_with_none_content_returns_empty_string(self) -> None:
        ...
        provider = OpenAIProvider(credentials=_openai_credentials(), transport_client=_NoneClient())
...
class TestEmbedErrorMapping:
    ...
    def test_401_maps_to_auth_error_naming_provider(self) -> None:
        ...
        # provider_openai.feature §"401 maps to AuthError" fails.
...
class TestDimensionAndHealth:
    ...
    def test_dimension_defaults_when_credentials_have_no_dims(self) -> None:
        ...
        provider = OpenAIProvider(
            credentials=_openai_credentials(dims=0),
            transport_client=_RecordingTransportClient(),
        )
    ...
    def test_dimension_uses_credential_dims_before_embed(self) -> None:
        provider = OpenAIProvider(
            credentials=_openai_credentials(dims=2048),
            transport_client=_RecordingTransportClient(),
        )
    ...
    def test_dimension_uses_observed_dim_after_first_embed(self) -> None:
        ...
        provider = OpenAIProvider(
            credentials=_openai_credentials(dims=1536),
            transport_client=client,
        )
    ...
    def test_healthcheck_ok_when_embed_succeeds(self) -> None:
        ...
        assert health.endpoint == "https://api.openai.com/v1"
