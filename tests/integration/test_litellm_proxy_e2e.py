"""Integration: litellm_proxy plugin embed + chat + error mapping end-to-end (#provider-plugin-arch IM-12).

Boundary chain:

  caller -> LiteLLMProxyProvider.embed_batch / .chat
        -> recording fake transport_client
           (the kwargs the plugin passes mirror what the openai-SDK
            ...
            ``Authorization: Bearer <virtual_key>`` by SDK contract)

  caller -> LiteLLMProxyProvider.embed_batch
        -> raising fake transport_client (APIStatusError shape)
        -> _map_transport_error -> RateLimited / AuthError / UpstreamError

This test ties the LiteLLM-proxy plugin together at the integration
...
def _credentials(
    *,
    api_key: str = "sk-int-litellm-vk",  # pragma: allowlist secret
...
def test_embed_round_trips_vectors_and_records_wire_shape() -> None:
    ...
    ``LiteLLMProxyProvider.embed_batch`` → recorded call has no ``input``,
    fails.
    """
...
def test_embed_passes_prefixed_model_name_through_verbatim() -> None:
    """Sabotage-proof: if the plugin started stripping LiteLLM's
    upstream prefix (``azure/``, ``bedrock/``, ``openai/``) from the
    model name, the proxy would fail to route the request to the
    configured upstream. This test pins the passthrough — distinguishes
    litellm_proxy operationally even though wire shape is OpenAI-compat.
    """
    transport = _RecordingTransport()
    provider = LiteLLMProxyProvider(
        credentials=_credentials(model="azure/foundry-embed-deploy"),
        transport_client=transport,
    )

    provider.embed_batch(["alpha"])

    assert transport.embeddings.calls[0]["model"] == "azure/foundry-embed-deploy"


def test_chat_round_trips_content_and_records_wire_shape() -> None:
    """Sabotage-proof: drop the ``messages=`` kwarg from
    ``LiteLLMProxyProvider.chat`` → recorded call missing messages, fails.
    """
    transport = _RecordingTransport(chat_content="hi from litellm")
    provider = LiteLLMProxyProvider(credentials=_credentials(), transport_client=transport)
...
def test_runtime_protocol_isinstance() -> None:
    ...
    dimension / healthcheck from ``LiteLLMProxyProvider`` breaks
