...
Tested through evaluate_sufficiency's public surface using a small fake
LLM backend. No @patch, no monkeypatch, no private imports.
"""
...
class _FakeLLMBackend:
    """Tiny LLM backend that returns a configured response regardless of input."""
...
def test_json_response_extracts_confidence() -> None:
    ...
    backend = _FakeLLMBackend(
        response='{"confidence": 0.85, "sufficient": true, "refined_query": null, "gaps": []}',
    )
...
def test_prose_response_extracts_confidence_via_regex_fallback() -> None:
    ...
    """
    backend = _FakeLLMBackend(
        response="Confidence: 0.7 — the results mostly cover the question but lack the Q1 update.",
    )
    result = evaluate_sufficiency(_state_with_chunks(), llm_backend=backend)
...
def test_percent_form_in_prose_extracted() -> None:
    ...
    backend = _FakeLLMBackend(response="My confidence is 70% based on the available evidence.")
    result = evaluate_sufficiency(_state_with_chunks(), llm_backend=backend)
    assert result["confidence"] == pytest.approx(0.7)
