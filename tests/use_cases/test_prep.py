...
# ---------------------------------------------------------------------------
# _default_chat production-default wiring (covers the 90% floor branches)
# ---------------------------------------------------------------------------


@pytest.mark.unit
def test_default_chat_raises_when_provider_unset() -> None:
    """The production default raises a typed ``ValueError`` when ``provider:``
    is missing from ``kairix.config.yaml``.

    Sabotage-proof: remove the ``if name is None: raise`` guard in
    ``_default_chat`` and the call below would either silently return ""
    or fall into ``get_provider(None)`` and raise an opaque
    ``ProviderNotRegistered`` — either way the typed-ValueError assertion
    here fails.
    """
    from kairix import paths as paths_mod
    from kairix.use_cases import prep as prep_mod

    original = paths_mod.provider_name
    paths_mod.provider_name = lambda: None  # type: ignore[assignment] — test seam, restored in finally
    try:
        with pytest.raises(ValueError, match="provider:"):
            prep_mod._default_chat(messages=[{"role": "user", "content": "x"}], max_tokens=10)
    finally:
        paths_mod.provider_name = original  # type: ignore[assignment] — restore test seam


@pytest.mark.unit
def test_default_chat_constructs_provider_chat_backend_when_provider_set() -> None:
    """Production-default path: resolves the provider, constructs
    ``ProviderChatBackend``, and delegates ``chat(messages, max_tokens=N)``.

    Sabotage-proof: drop the ``backend.chat(**kwargs)`` line and the
    assertion below catches the missing call (FakeProvider records every
    chat invocation).
    """
    from kairix import paths as paths_mod
    ...
    fake = FakeProvider(name="fake_provider", chat_reply="ok-from-fake")
    original_name = paths_mod.provider_name
    original_get = providers_mod.get_provider
    paths_mod.provider_name = lambda: "fake_provider"  # type: ignore[assignment] — test seam, restored in finally
    providers_mod.get_provider = lambda name, registry=None: fake  # type: ignore[assignment] — test seam, restored in finally
    try:
        result = prep_mod._default_chat(messages=[{"role": "user", "content": "hi"}], max_tokens=10)
        assert result == "ok-from-fake"
        assert len(fake.chat_calls) == 1
        assert fake.chat_calls[0]["messages"] == [{"role": "user", "content": "hi"}]
        assert fake.chat_calls[0]["max_tokens"] == 10
    finally:
        paths_mod.provider_name = original_name  # type: ignore[assignment] — restore test seam
        providers_mod.get_provider = original_get  # type: ignore[assignment] — restore test seam
