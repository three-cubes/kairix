...
func TestServiceRunHappyPath(t *testing.T) {
	r := newFakeRunner(map[string]fakeResponse{
		"docker compose pull kairix kairix-worker":                                             {out: []byte("Pulled")},
		"docker compose up -d kairix kairix-worker":                                            {out: []byte("Started")},
...
func TestServiceRunPullFailure(t *testing.T) {
	r := newFakeRunner(map[string]fakeResponse{
		"docker compose pull kairix kairix-worker": {out: []byte("error"), err: errors.New("network down")},
	})
